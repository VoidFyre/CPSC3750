...
function suggest(currentSearch, searchStr) {
    ...
    switch (currentSearch) {
        case "class":
            while (classHints.firstChild) {
                classHints.removeChild(classHints.lastChild);
            }
            ...
            break;
        case "race":
            while (raceHints.firstChild) {
                raceHints.removeChild(raceHints.lastChild);
            }
            ...
            break;
        case "type":
            while (typeHints.firstChild) {
                typeHints.removeChild(typeHints.lastChild);
            }
            ...
            break;
        case "faction":
            while (factionHints.firstChild) {
                factionHints.removeChild(factionHints.lastChild);
            }
            ...
            break;
        case "quality":
            while (qualityHints.firstChild) {
                qualityHints.removeChild(qualityHints.lastChild);
            }
...
function findSingleCard(cardId) {
    ...
    xhr.addEventListener('readystatechange', function () {
    	if (this.readyState === this.DONE) {
            ...
            str = "<p><b>Card Id:</b> " + card.cardId + "</p>";
            str += "<p><b>dbfId:</b> " + card.dbfId + "</p>";
            str += "<p><b>Name:</b> " + card.name + "</p>";
            
            if (card.img) str += "<h2>Images:</h2><br><img class=\"full\" src=\"" + card.img + "\" alt=\"Image File Invalid\">";
            if (card.imgGold) str += "<img class=\"full\" src=\"" + card.imgGold + "\" alt=\"Image File Invalid\">";
            
            if (card.cardSet) str += "<p><b>Card Set:</b> " + card.cardSet + "</p>";
            if (card.type) str += "<p><b>Type:</b> " + card.type + "</p>";
            if (card.rarity) str += "<p><b>Rarity:</b> " + card.rarity + "</p>";
            if (card.cost) str += "<p><b>Cost:</b> " + card.cost + "</p>";
            if (card.attack) str += "<p><b>Attack:</b> " + card.attack + "</p>";
            if (card.health) str += "<p><b>Health:</b> " + card.health + "</p>";
            if (card.elite) str += "<p><b>Elite:</b> " + card.elite + "</p>";
            if (card.race) str += "<p><b>Race:</b> " + card.race + "</p>";
            if (card.playerClass) str += "<p><b>Player Class:</b> " + card.playerClass + "</p>";
            if (card.text) str += "<p><b>Text:</b> " + card.text + "</p>";
            if (card.flavor) str += "<p><b>Flavor:</b> " + card.flavor + "</p>";
            
            if (userId != "0") {
                if (checkFavorite(card.cardId)) {
                str += "<span class=\"center-span\"><button style=\"width: 100%;\" id=\"unfavorite\" onclick=\"removeFavorite('" + card.cardId + "');\">Unfavorite</button></span>";
                } else {
                    str += "<span class=\"center-span\"><button style=\"width: 100%;\" id=\"favorite\" onclick=\"addFavorite('" + card.cardId + "');\">Favorite</button></span>";
                }
            } else {
                str += "<span class=\"center-span\"><a class=\"buttonLink\" href=\"https://voidfyre.space/login?prevURL=https://voidfyre.space/cpsc3750/collectApp\">Log In</a></span>";
            }
